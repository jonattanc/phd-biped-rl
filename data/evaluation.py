...
def analisar_dataframe_completo(df, nome_arquivo):
    ...
    for coluna in colunas_validas:
        ...
        estatisticas = {
            ...
            'Média': dados.mean(),
            'DP': dados.std(),
            'EP': dados.std() / np.sqrt(len(dados)) if len(dados) > 0 else np.nan,
    ...
    if len(colunas_validas) >= 2:
        ...
        try:
            ...
            comparacoes_grupos.append({
                ...
                'p-valor': p_levene,
                'Resultado': 'Homogêneas' if p_levene > 0.05 else 'Não Homogêneas',
                'n': N_total
            })
        except:
            pass
                    ...
                    for i in range(len(tukey_result.summary().data) - 1):
                        ...
                        if len(row) > 6:
                            comparacao = {
                                ...
                                'p-valor ajustado': float(row[5]) if len(row) > 5 else np.nan,
                                'Significativo': bool(row[6]) if len(row) > 6 else False,
                                'Tipo': 'Tukey HSD'
                            }
                            resultados_tukey_completo.append(comparacao)
            except Exception as e:
                resultados_tukey_completo.append({
                    'Arquivo': nome_arquivo,
                    'Grupo 1': 'Erro',
                    'Grupo 2': str(e)[:50],
                    'Tipo': 'Erro Tukey'
    ...
    return {
        ...
        'tukey': resultados_tukey_completo,
...
def criar_excel_por_arquivo(arquivos_resultados):
    ...
    for nome_arquivo, resultados in arquivos_resultados.items():
        ...
        if resultados['normalidade']:
            ...
            ws.append(["Variável", "N", "Estatística W", "p-valor", "Resultado"])
        ...
        if resultados['comparacoes']:
            ...
            for comp in resultados['comparacoes']:
                if comp['Teste'] == 'Levene':
                    ws.append(["Teste de Levene (Homogeneidade de Variâncias)"])
        ...
        if resultados['tukey'] and any(r['Tipo'] == 'Tukey HSD' for r in resultados['tukey']):
            ws.append(["4. TESTE POST-HOC TUKEY HSD (Todas as Comparações)"])
            ws.append(["Grupo 1", "Grupo 2", "Diferença", "Erro Padrão", 
                      "IC 95% Inferior", "IC 95% Superior", "p-valor", "Significativo"])
            
            tukey_rows = [r for r in resultados['tukey'] if r['Tipo'] == 'Tukey HSD']
            for tukey in tukey_rows:
                ws.append([
                    tukey['Grupo 1'],
                    tukey['Grupo 2'],
                    tukey['Diferença'],
                    tukey['Erro Padrão'],
                    tukey['IC 95% Inferior'],
                    tukey['IC 95% Superior'],
                    tukey['p-valor ajustado'],
                    "SIM" if tukey['Significativo'] else "NÃO"
                ])
    ...
    # Criar aba com todos os resultados Tukey consolidados
    criar_aba_tukey_consolidado(wb, arquivos_resultados)
    
    # Salvar arquivo
    nome_excel = 'Analise_Estatistica_Completa.xlsx'
    wb.save(nome_excel)
...
def criar_aba_tukey_consolidado(wb, arquivos_resultados):
    """Cria aba com todos os resultados Tukey consolidados"""
    ws_tukey = wb.create_sheet(title="TUKEY CONSOLIDADO")
    
    ws_tukey.append(["RESULTADOS TUKEY HSD CONSOLIDADOS"])
    ws_tukey.append([f"Data: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"])
    ws_tukey.append([])
    ws_tukey.append(["Arquivo", "Grupo 1", "Grupo 2", "Diferença", "Erro Padrão", 
                    "IC 95% Inferior", "IC 95% Superior", "p-valor", "Significativo"])
    
    # Coletar todos os resultados Tukey
    todos_tukey = []
    for nome_arquivo, resultados in arquivos_resultados.items():
        for tukey in resultados['tukey']:
            if tukey['Tipo'] == 'Tukey HSD':
                tukey['Arquivo'] = nome_arquivo
                todos_tukey.append(tukey)
    
    # Ordenar por significância e valor-p
    todos_tukey.sort(key=lambda x: (not x['Significativo'], x['p-valor ajustado']))
    
    for tukey in todos_tukey:
        ws_tukey.append([
            tukey['Arquivo'],
            tukey['Grupo 1'],
            tukey['Grupo 2'],
...
def criar_relatorio_texto(resumo_arquivos):
    """Cria um relatório resumido em texto"""
    with open('Resumo_Executivo.txt', 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write("RELATÓRIO EXECUTIVO - ANÁLISE ESTATÍSTICA\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write("📊 RESUMO NUMÉRICO:\n")
        f.write("-" * 40 + "\n")
        f.write(f"• Total de arquivos analisados: {total_arquivos}\n")
        f.write(f"• Total de variáveis analisadas: {total_variaveis}\n")
        f.write(f"• Total de observações: {total_observacoes}\n")
        ...
        f.write(f"\n📈 RESULTADOS DOS TESTES:\n")
        f.write("-" * 40 + "\n")
        f.write(f"• Arquivos com ANOVA significativa: {arquivos_anova_sig}/{total_arquivos}\n")
        f.write(f"• Arquivos com Kruskal-Wallis significativo: {arquivos_kw_sig}/{total_arquivos}\n")
        ...
        for resumo in resumo_arquivos:
            f.write(f"\n📄 {resumo['arquivo']}\n")
            f.write(f"   Variáveis: {resumo['variaveis']} | ")
            f.write(f"Observações: {resumo['observacoes']} | ")
            f.write(f"ANOVA: {'✓' if resumo['anova_sig'] else '✗'} | ")
            f.write(f"K-W: {'✓' if resumo['kw_sig'] else '✗'}\n")
        
        f.write(f"\n" + "=" * 70 + "\n")
        f.write("🎯 RECOMENDAÇÕES:\n")
        f.write("=" * 70 + "\n\n")
        
        if arquivos_anova_sig > 0:
            f.write("1. Foram encontradas diferenças significativas em alguns arquivos.\n")
            f.write("   → Consulte a aba 'TUKEY CONSOLIDADO' para ver todas as comparações.\n")
        
        if arquivos_kw_sig > arquivos_anova_sig:
            f.write("2. Kruskal-Wallis foi mais frequente que ANOVA.\n")
            f.write("   → Considere que os dados podem não ser normalmente distribuídos.\n")
        
        f.write(f"\n3. Para análise detalhada, consulte o arquivo Excel gerado.\n")
        f.write("   Cada arquivo CSV tem sua própria aba com análise completa.\n")
...
def processar_todos_csv():
    ...
    for arquivo in arquivos_csv:
        try:
            ...
            var_count = len(resultados['estatisticas'])
            tukey_count = len([r for r in resultados['tukey'] if r['Tipo'] == 'Tukey HSD'])
            
            print(f"  ✓ Variáveis analisadas: {var_count}")
            if tukey_count > 0:
                print(f"  ✓ Comparações Tukey: {tukey_count}")
            print()
    ...
    if arquivos_resultados:
        ...
        criar_relatorio_texto(resumo_arquivos)
        
        print("=" * 60)
        print("✅ ANÁLISE CONCLUÍDA COM SUCESSO!")
        print("=" * 60)
        print("\n📁 ARQUIVOS GERADOS:")
        print(f"  1. {excel_file} (Excel completo com todas as análises)")
        print(f"  2. Resumo_Executivo.txt (Relatório executivo)")
        
        print("\n📊 ESTRUTURA DO EXCEL:")
        print(f"  • ÍNDICE - Visão geral de todos os arquivos")
        print(f"  • RESUMO GERAL - Estatísticas consolidadas")
        print(f"  • TUKEY CONSOLIDADO - Todas as comparações Tukey")
        
        for arquivo in arquivos_resultados.keys():
            nome_aba = arquivo[:31]
            print(f"  • {nome_aba} - Análise completa do arquivo")
        
        print(f"\n📈 TOTAL: {len(arquivos_resultados)} abas de análise")
