...
def calcular_metricas_gerais():
    """Calcula métricas gerais para cada arquivo CSV incluindo CC como método geral e separado por pista"""
    
    arquivos_csv = glob.glob("*.csv")
    
    # Remover arquivos de resultados anteriores para não analisá-los
    arquivos_csv = [f for f in arquivos_csv if not f.startswith(('tabela_', 'estatisticas_', 'analise_', 'teste_t_', 'painel_'))]
    ...
    for arquivo in arquivos_csv:
        try:
            ...
            # Processar arquivo normalmente (incluindo CC como método geral)
            resultado_geral = calcular_metricas_arquivo(df, nome_categoria)
            if resultado_geral:
                resultados.append(resultado_geral)
            
            # Se for o arquivo CC, também separar por pista para análises detalhadas
            if nome_categoria == 'CC':
                dados_cc_separados = separar_cc_por_pista(df)
                
                # Processar cada pista do CC separadamente
                for pista, df_pista in dados_cc_separados.items():
                    nome_categoria_pista = f'CC_{pista}'
                    resultado_pista = calcular_metricas_arquivo(df_pista, nome_categoria_pista)
                    if resultado_pista:
                        resultados.append(resultado_pista)
...
def calcular_metricas_arquivo(df, nome_categoria):
    """Calcula métricas para um DataFrame específico"""
...
def gerar_tabelas_formatadas(df_resultados):
    ...
    for _, row in df_resultados.iterrows():
        # Formatar números com 3 casas decimais e vírgula como separador
        media_distancia = f"{row['Media_Distancia']:.3f}".replace('.', ',')
        desvio_distancia = f"{row['Desvio_Padrao_Distancia']:.3f}".replace('.', ',')
        maior_recompensa = f"{row['Maior_Recompensa']:.3f}".replace('.', ',')
        tempo_total = f"{row['Tempo_Total_Treinamento']:.3f}".replace('.', ',')
        melhor_tempo = f"{row['Melhor_Tempo_9m']:.3f}".replace('.', ',') if row['Melhor_Tempo_9m'] != 'N/A' else 'N/A'
        percentual_sucessos = f"{row['Percentual_Sucessos_Pos_9m']:.3f}".replace('.', ',')
        velocidade_media = f"{row['Velocidade_Media_Sucessos']:.3f}".replace('.', ',')
        eficiencia_rp = f"{row['Eficiencia_RP']:.3f}".replace('.', ',')
        eficiencia_dp = f"{row['Eficiencia_DP']:.3f}".replace('.', ',')
        eficiencia_vt = f"{row['Eficiencia_VT']:.3f}".replace('.', ',')
