...
def process_runner(selected_environment, selected_robot, algorithm, ipc_queue, pause_value, exit_value, enable_real_time_value, device="cpu"):
    ...
    try:
        ...
        while timesteps_completed < total_timesteps and not exit_value.value:
            # VERIFICAÇÃO DE COMANDOS VIA ARQUIVO (alternativa à IPC)
            try:
...
def process_runner_resume(selected_environment, selected_robot, algorithm, ipc_queue, pause_value, exit_value, enable_real_time_value, device="cpu", model_path=None):
    ...
    try:
        ...
        agent = Agent(logger, model_path=model_path, device=device)
