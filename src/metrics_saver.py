...
def calculate_stability_metrics(orientation_history):
    ...
    if not orientation_history:
        return {
            "stability_roll": 0, "stability_pitch": 0, "stability_yaw": 0,
            "overall_stability": 0, "max_roll": 0, "max_pitch": 0
        }
...
def calculate_simple_confidence(times, sigma_multiplier=2):
    ...
    return {
        "lower": max(0, mean_time - margin_error),
        "upper": mean_time + margin_error,
        "mean": mean_time,
        "margin_error": margin_error
    }
...
def compile_comprehensive_metrics(episode_data, joint_data, orientation_data, foot_contact_data):
    ...
    # Métricas de marcha
    gait_metrics = calculate_gait_metrics(
        foot_contact_data["history"],
        foot_contact_data.get("timestep", 0.02)
    )
