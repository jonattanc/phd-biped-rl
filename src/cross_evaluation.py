...
class CrossEvaluation:
    ...
    def _run_complexity_analysis(self, num_episodes, deterministic):
        ...
        for circuit in self.complexity_order:
            ...
            metrics = self._evaluate_model_on_circuit(
                model_path, circuit, "robot_stage1", num_episodes, deterministic
            )
    ...
    def _run_generalization_analysis(self, num_episodes, deterministic):
        ...
        for origin_circuit in self.complexity_order:
            ...
            for target_circuit in self.complexity_order:
                ...
                metrics = self._evaluate_model_on_circuit(
                    origin_model, target_circuit, "robot_stage1", num_episodes, deterministic
                )
    ...
    def _run_specificity_analysis(self, num_episodes, deterministic):
        ...
        for target_circuit in self.complexity_order:
            ...
            ae_metrics = self._evaluate_model_on_circuit(
                ae_model, target_circuit, "robot_stage1", num_episodes, deterministic
            )
            ...
            for origin_circuit in self.complexity_order:
                ...
                ag_metrics = self._evaluate_model_on_circuit(
                    ag_model, target_circuit, "robot_stage1", num_episodes, deterministic
                )
    ...
    def _find_specialist_model(self, circuit_name):
        ...
        possible_patterns = [
            f"model_{circuit_name}.zip",
            f"{circuit_name}_model.zip", 
            f"specialist_{circuit_name}.zip"
        ]
    ...
    def _analyze_generalization_patterns(self, generalization_results):
        ...
        for result in generalization_results:
            ...
            patterns[origin][target] = {
                "avg_time": result["avg_time_target"],
                "success_rate": result["success_rate_target"]
            }
    ...
    def _calculate_specificity_gaps(self, specificity_results):
        ...
        for circuit, gap_list in gaps.items():
            gap_stats[circuit] = {
                ...
                "std_gap": np.std(gap_list) if len(gap_list) > 1 else 0,
