...
class RewardTab:
    ...
    def create_default_config(self):
        ...
        default_config = {
            "metadata": {
                "name": "default",
                "version": "1.0",
                "description": "Configuração balanceada para locomoção básica",
    ...
    def setup_ui(self):
        ...
        ttk.Label(editor_frame, text="Críticos (Sobrevivência)", font=('Arial', 10, 'bold')).grid(
            row=0, column=0, columnspan=5, sticky=tk.W, pady=(10, 5), padx=5)
        ...
        # EFICIÊNCIA
        ttk.Label(editor_frame, text="Eficiência", font=('Arial', 10, 'bold')).grid(
            row=row_idx, column=0, columnspan=5, sticky=tk.W, pady=(15, 5), padx=5)
        row_idx += 1

        efficiency_components = [
            ("effort_penalty", "Penalidade Esforço", -5, 0),
            ("jerk_penalty", "Penalidade Agito", -5, 0)
        ]
        ...
        # NAVEGAÇÃO
        ttk.Label(editor_frame, text="Navegação", font=('Arial', 10, 'bold')).grid(
            row=row_idx, column=0, columnspan=5, sticky=tk.W, pady=(15, 5), padx=5)
        row_idx += 1

        navigation_components = [
            ("center_bonus", "Bônus Centro", 0, 200),
            ("warning_penalty", "Penalidade por Zona", -100, 0)
        ]
        ...
        # AVANÇADOS
        ttk.Label(editor_frame, text="Avançados", font=('Arial', 10, 'bold')).grid(
            row=row_idx, column=0, columnspan=5, sticky=tk.W, pady=(15, 5), padx=5)
        row_idx += 1

        advanced_components = [
            ("gait_regularity", "Regularidade da Marcha", 0, 50),
            ("symmetry_bonus", "Bônus de Simetria", 0, 50),
            ("clearance_bonus", "Bônus Elevação Pé", 0, 50)
        ]
    ...
    def _create_component_row(self, parent, comp_id, label, min_val, max_val, row, default_enabled=True):
        ...
        # Bind events
        var.trace('w', lambda *args, cid=comp_id, lbl=value_label, v=var, ent=value_entry: 
                 self.on_scale_change(cid, lbl, v, ent))
        value_entry.bind('<Return>', lambda e, cid=comp_id, v=var, ent=value_entry: 
                        self.on_entry_change(cid, v, ent))
        enabled_var.trace('w', lambda *args, cid=comp_id, ev=enabled_var: 
                         self.on_enabled_change(cid, ev))

        self.quick_editors[comp_id] = {
            'var': var,
            'scale': scale,
            'label': value_label,
            'entry': value_entry,
