...
class ComparisonTab:
    ...
    def setup_ui(self):
        ...
        ttk.Button(buttons_frame, text="Adicionar Modelo", command=self.add_comparison_model, width=15).pack(pady=2, fill=tk.X)
        ttk.Button(buttons_frame, text="Remover Selecionado", command=self.remove_comparison_model, width=15).pack(pady=2, fill=tk.X)
        ttk.Button(buttons_frame, text="Limpar Lista", command=self.clear_comparison_models, width=15).pack(pady=2, fill=tk.X)
        ...
        env_combo = ttk.Combobox(settings_frame, textvariable=self.comparison_env_var, values=["PR", "Pmu", "RamA", "RamD", "PG", "PRB"], width=10)
        ...
        robot_combo = ttk.Combobox(settings_frame, textvariable=self.comparison_robot_var, values=["robot_stage1", "robot_stage2", "robot_stage3"], width=12)
        ...
        self.run_comparison_btn = ttk.Button(button_frame, text="Executar Comparação", command=self.run_comparison, width=20)
        ...
        ttk.Button(export_frame, text="Exportar Resultados", command=self.export_comparison_results).pack(side=tk.LEFT, padx=5)
        ttk.Button(export_frame, text="Exportar Gráficos", command=self.export_comparison_plots).pack(side=tk.LEFT, padx=5)
    ...
    def _run_comparison(self, models, episodes):
        ...
        try:
            ...
            for i, model in enumerate(models):
                ...
                metrics = evaluate_and_save(model_path=model["path"], circuit_name=environment, avatar_name=robot, num_episodes=episodes, seed=42)

                if metrics:
                    comparison_results.append({"name": model["name"], "metrics": metrics, "path": model["path"]})
                else:
    ...
    def _update_comparison_plots(self, results):
        ...
        try:
            ...
            for bar, value in zip(bars1, success_rates):
                ...
                self.axs_comparison[0, 0].text(bar.get_x() + bar.get_width() / 2.0, height, f"{value:.1f}%", ha="center", va="bottom")
            ...
            for bar, value in zip(bars2, avg_times):
                height = bar.get_height()
                self.axs_comparison[0, 1].text(bar.get_x() + bar.get_width() / 2.0, height, f"{value:.2f}s", ha="center", va="bottom")
            ...
            for bar, value in zip(bars3, std_times):
                height = bar.get_height()
                self.axs_comparison[1, 0].text(bar.get_x() + bar.get_width() / 2.0, height, f"{value:.2f}s", ha="center", va="bottom")
            ...
            for bar, value in zip(bars4, performance_scores):
                height = bar.get_height()
                self.axs_comparison[1, 1].text(bar.get_x() + bar.get_width() / 2.0, height, f"{value:.1f}%", ha="center", va="bottom")
    ...
    def export_comparison_plots(self):
        ...
        try:
            ...
            if directory:
                ...
                fig_combined.savefig(os.path.join(directory, f"comparison_combined_{timestamp}.png"), dpi=300, bbox_inches="tight")
                ...
                for plot_type in plot_types:
                    ...
                    fig_single.savefig(os.path.join(plots_dir, f"{plot_type}_{timestamp}.png"), dpi=300, bbox_inches="tight")
    ...
    def _plot_to_export_figure(self, axs, results):
        ...
        for bar, value in zip(bars1, success_rates):
            height = bar.get_height()
            axs[0, 0].text(bar.get_x() + bar.get_width() / 2.0, height, f"{value:.1f}%", ha="center", va="bottom", fontsize=8)
